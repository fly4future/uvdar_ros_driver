...
bool UvdarRosDriver::openSerialPort(std::string serial_number, int baudrate) {
  ...
  if (serial_number.empty()) {
    ROS_ERROR_THROTTLE(
        1.0, "[%s]: Serial number is not set, available device serial numbers:", ros::this_node::getName().c_str());
  ...
  ROS_INFO_STREAM_THROTTLE(1.0, std::string(ros::this_node::getName().length() + 4, ' ')
                                    << "SerialNumber: " << serial_number);
  portname = getDeviceSerialPort(serial_number);
  if (portname.empty()) {
    ROS_ERROR_THROTTLE(1.0, "[%s]: Device not found, it's serial port does not exist",
...
void UvdarRosDriver::serialThread(void) {
  ...
  while (running_) {
    ...
    if (!connected) {
      ROS_WARN("[%s]: Terminating serial thread because the serial port was disconnected",
               ros::this_node::getName().c_str());
      return;
    }
    ...
    if (bytes_read > 0 && bytes_read < SERIAL_BUFFER_SIZE) {
      /* If the serial device is disconnected and readSerial() is called, it will return max_int number of read bytes,
       * thats why we check it against the SERIAL_BUFFER_SIZE */
      ...
      for (uint16_t i = 0; i < bytes_read; i++) {
        ...
        if (llcp_processChar(rx_buffer[i], &llcp_receiver, &message_in, &checksum_matched)) {
          /* ROS_INFO_STREAM("[UvdarRosDriver]: received message with id " << message_in->id << "; checksum is: " <<
           * checksum_matched); */
          ...
          std::vector<msg_counter>::iterator it =
              std::find_if(received_msgs.begin(), received_msgs.end(),
                           boost::bind(&msg_counter::init, _1) == msg_out.range.initiator_address &&
...
void UvdarRosDriver::callbackMaintainerTimer(const ros::TimerEvent& event) {
  ...
  for (size_t i = 0; i < received_msgs.size(); i++) {
    std::string tmp_string = "initiator " + std::to_string(received_msgs[i].init) + ", responder " +
                             std::to_string(received_msgs[i].resp) + ": " + std::to_string(received_msgs[i].num) +
                             " msgs";
